...
export class BungieApi {
  ...
  async requestPlatform (options) {
    ...
    const response = await this.got({
      ...
      searchParams: prepareSearchParams(options.searchParams)
    })
...
function autoRefresh (api) {
  ...
  if (!refreshToken || !refreshToken.valid) {
    throw new Error('Expected a valid refresh token')
  }
    ...
    hooks: {
      ...
      afterResponse: [
        async (response, retry) => {
          // TODO: not sure this is the correct error code
          if (Object(response.body).ErrorCode === PlatformErrorCodes.OAuthAccessTokenExpired) {
            await refresh()
            return retry({
              headers: {
